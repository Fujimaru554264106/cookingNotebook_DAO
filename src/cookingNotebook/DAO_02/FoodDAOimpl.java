...
public class FoodDAOimpl implements FoodDAO {
	...
	public List<Food> getAll() throws SQLException {
		...
		if(rs != null) {
			...
			while(rs.next()) {
				l.add(new Food(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getString(4), rs.getString(5)));
			}
		}
		rs.close();
		ps.close();
	...
	public List<Food> getListFood(int mid) throws SQLException {
		Connection c = Database.getConnection();
		List<Food> l = null;
		String sql = "SELECT Fid, Fname, CookTime, Ftype, Fimage FROM V2_Menu_Food WHERE Mid = ?";
		PreparedStatement ps = c.prepareStatement(sql);
		ps.setInt(1, mid);
		ResultSet rs = ps.executeQuery();
		if(rs != null) {
